...
contract Loan is ZkAssetMintable {
  ...
  // A note struct which stores an address and bytes32 datatype
  struct Note {
    address owner;
    bytes32 noteHash;
  }
...
// The settle loan function takes as input the proofdata, _currentInterestBalance and _lender
// does a onlyLoanDapp check to make sure the lender(msg.sender) is not the loan dapp i.e the address that called the constructor
//  then process the loan settlement
// some loanVariable attriburtes are then updated like the loanSettlementDate, lastInterestPaymentDate,currentInterestBalance and lender
  function settleLoan(
...
// does a onlyLoanDapp check to make sure the lender(msg.sender)is not the loan dapp i.e the contract creator
